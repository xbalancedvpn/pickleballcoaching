-- Pickyla v17-G legacy two-person client split helper
-- Keeps historical bookings/payments intact while turning one combined client identity into two client profiles.
...
begin
  ...
  -- Do not accidentally rename the legacy profile over an existing active primary client.
  ...
  -- Reuse an existing Player 2 profile whenever the match is unambiguous.
  if v_s_contact_key is not null then
    ...
    select count(*), min(id) into v_same_name_count, v_secondary_id
    from public.clients
    where id <> p_client_id
      and is_active = true
      and name_key = v_s_name_key;
  ...
  -- Convert the old combined profile into Player 1.
  update public.clients
